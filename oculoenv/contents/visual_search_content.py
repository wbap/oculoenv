...
class VisualSearchSignSprite(object):
  def __init__(self,
    ...
    self.pos_x = -self.width * GRID_DIVISION + self.width * (1 + 2 * x_index)
    self.pos_y = -self.width * GRID_DIVISION + self.width * (1 + 2 * y_index)
...
class VisualSearchContent(BaseContent):
  ...
  def _init(self):
    ...
    sign_pathes = ['general_t0.png', 'general_t1.png', 'general_l0.png', 'general_l1.png',
                   'general_r0.png', 'general_r1.png', 'general_s0.png', 'general_z0.png']
    self.sign_textures = self._load_textures(sign_pathes)
    
    self.start_sprite = ContentSprite(start_marker_texture, 0.0, 0.0,
                                      START_MARKER_WIDTH)

    self.button_sprite_no = ContentSprite(white_texture, -0.2, -0.9, BUTTON_WIDTH,
                                        color=[0.0,0.0,0.0]) # Left
    self.button_sprite_yes = ContentSprite(white_texture,  0.2, -0.9, BUTTON_WIDTH,
                                        color=[0.0,0.0,0.0]) # Right
  ...
  def _get_sign_variables(self, pos_index, distraction_type, has_target):
    ...
    if has_target and pos_index == 0:
      ...
      if distraction_type == DISTRACTION_TYPE_COLOR:
        ...
        index = np.random.randint(1, len(self.sign_textures) * len(COLORS))
  ...
  def _prepare_sign_sprites(self):
    ...
    for i in range(sign_size):
      tex_index, color_index = self._get_sign_variables(i, distraction_type, has_target)
      sign_sprite = VisualSearchSignSprite(self.sign_textures,
                                           tex_index,
                                           pos_indices[i],
                                           color_index)
