...
class TestMatrix4(unittest.TestCase):
  ...
  def test_set_rot_x(self):
    ...
    for i in range(8):
      ...
      ret = np.allclose([0.0, math.sin(-angle), math.cos(-angle), 1.0], v1)
      self.assertTrue(ret)
  ...
  def test_set_rot_y(self):
    ...
    for i in range(8):
      ...
      v1 = mat0.transform(v0)
      ret = np.allclose([math.cos(-angle), 0.0, math.sin(-angle), 1.0], v1)
      self.assertTrue(ret)
  ...
  def test_set_rot_z(self):
    ...
    for i in range(8):
      ...
      v1 = mat0.transform(v0)
      ret = np.allclose([math.sin(-angle), math.cos(-angle), 0.0, 1.0], v1)
      self.assertTrue(ret)
